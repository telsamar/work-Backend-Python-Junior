...
def assign_category(warehouse_profit):
    conditions = [
        (warehouse_profit['accumulated_percent_profit_product_of_warehouse'] <= 70),
        (warehouse_profit['accumulated_percent_profit_product_of_warehouse'] > 70) & (warehouse_profit['accumulated_percent_profit_product_of_warehouse'] <= 90)
    ]
    choices = ['A', 'B']
    warehouse_profit['category'] = np.select(conditions, choices, default='C')

    return warehouse_profit

if __name__ == "__main__":
    warehouse_summary = compute_warehouse_profit('trial_task.json')
    warehouse_with_accumulated_percent = compute_accumulated_percent(warehouse_summary)
    result = assign_category(warehouse_with_accumulated_percent)
    print(result)
